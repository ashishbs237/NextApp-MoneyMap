...
export async function PUT(
    ...
    try {
        ...
        const updated = await InvestmentSchema.findByIdAndUpdate(
            id,
            { amount, label, tag },
            { new: true }
        );
        ...
        return NextResponse.json(
            { message: "Investment item updated", data: updated },
            { status: 200 }
        );
    } catch (err: any) {
        const errorMessage =
...
export async function DELETE(
    ...
    try {
        ...
        const deleted = await InvestmentSchema.findByIdAndDelete(id);
        ...
        return NextResponse.json({
            data: { ...deleted, message: "Investment item deleted" },
            status: 200,
        });
    } catch (err: any) {
        const errorMessage =
